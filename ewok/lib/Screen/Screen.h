...
public:
	int16_t vals[N];
	constexpr Screen(const std::initializer_list<const char*>&);
	void begin();

};

template <int N>
constexpr Screen<N>::Screen(const std::initializer_list<const char*>& init) : fields(init) {
...
template <int N>
void Screen<N>::handleField(const int& index) {
